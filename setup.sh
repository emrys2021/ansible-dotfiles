...
# 重新生成 APT 镜像源配置。
# 这里会先备份 /etc/apt/sources.list 和 /etc/apt/sources.list.d，
# 再停用 sources.list.d 里的旧 .list/.sources 文件，避免 cdrom、旧镜像或第三方源继续影响 apt update。
configure_apt_mirror_sources() {
    ...
    backup_dir="/etc/apt/ansible-dotfiles-sources-backup-$(date +%Y%m%d%H%M%S)"

    run_sudo mkdir -p "$backup_dir"

    if [ -f "/etc/apt/sources.list" ]; then
        run_sudo cp /etc/apt/sources.list "$backup_dir/sources.list"
    fi

    if [ -d "/etc/apt/sources.list.d" ]; then
        run_sudo cp -a /etc/apt/sources.list.d "$backup_dir/sources.list.d"
        run_sudo find /etc/apt/sources.list.d -type f \( -name "*.list" -o -name "*.sources" \) -exec mv {} "$backup_dir/" \;
    fi

    run_sudo tee /etc/apt/sources.list > /dev/null <<EOF
# Generated by ansible-dotfiles setup.sh
